...
release-push:
    ...
    echo "📄 Refreshing example showcase asset"
    just copy-example
...
github-release version="":
    ...
    if [[ -z "{{ version }}" ]] || [[ "$TARGET_TAG" == $(git tag --sort=-version:refname | head -n 1) ]]; then
        ...
        # Upload example HTML showcase asset (generated by `just copy-example`).
        # Warn loudly rather than skip silently — a missing asset here is what
        # caused it to drop off releases after 1.0.0.
        if [[ -f "release-assets/claude-code-log-transcript.html" ]]; then
            echo "📄 Uploading example HTML transcript"
            gh release upload "$TARGET_TAG" "release-assets/claude-code-log-transcript.html" --clobber
        else
            echo "⚠️  release-assets/claude-code-log-transcript.html missing — run 'just copy-example' to refresh the showcase asset. Skipping its upload."
        fi
    fi
...
# Copy the example transcript (and its cache) used as the GitHub release
# showcase asset into release-assets/. The source lives on the maintainer's
# machine; if it's missing this warns and skips rather than failing, so it
# never blocks a release elsewhere. Kept out of docs/ so it doesn't get swept
# into the MkDocs site.
copy-example:
    #!/usr/bin/env bash
    set -uo pipefail
    SRC_DIR="$HOME/.claude/projects/-Users-dain-workspace-claude-code-log"
    DEST_DIR="release-assets"
    mkdir -p "$DEST_DIR"
    if [[ -f "$SRC_DIR/combined_transcripts.html" ]]; then
        rsync "$SRC_DIR/combined_transcripts.html" "$DEST_DIR/claude-code-log-transcript.html"
        rsync -a "$SRC_DIR/cache" "$DEST_DIR/" 2>/dev/null || true
        echo "📄 Example transcript copied to $DEST_DIR/claude-code-log-transcript.html"
    else
        echo "⚠️  Example source not found at $SRC_DIR/combined_transcripts.html"
        echo "    Run claude-code-log on this project to generate it, then re-run 'just copy-example'."
    fi
